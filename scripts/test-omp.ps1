...
Write-Host "=== Prueba del Filtro Sobel con OpenMP ===" -ForegroundColor Green
...
Write-Host ""
Write-Host "Compilando y probando la versión OpenMP..." -ForegroundColor Yellow
...
Write-Host "Paso 6: Ejecutar prueba OpenMP..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "=== Prueba OpenMP exitosa ===" -ForegroundColor Green
    Write-Host "La versión multihilo funciona correctamente" -ForegroundColor White
    Write-Host "Imágenes copiadas a: images/" -ForegroundColor Green
    
    # Mostrar archivos generados
    Write-Host ""
    Write-Host "Archivos generados:" -ForegroundColor Yellow
    Get-ChildItem images\*omp*.jpg -ErrorAction SilentlyContinue | ForEach-Object {
        Write-Host "  ✅ $($_.Name)" -ForegroundColor Green
    }
} else {
    Write-Host ""
    Write-Host "=== Error en la prueba OpenMP ===" -ForegroundColor Red
