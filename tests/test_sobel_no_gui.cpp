...
int main() {
    std::cout << "=== Prueba del Filtro Sobel (Sin GUI) ===" << std::endl;
    ...
    std::cout << "Resultado Sobel: " << sobelResult.cols << "x" << sobelResult.rows << " (1 canal)" << std::endl;
    
    // Calcular estadísticas del resultado
    double minVal, maxVal;
    cv::minMaxLoc(sobelResult, &minVal, &maxVal);
